  ...
  return (value: unknown) => {
    ...
    if (value && typeof value === 'string' && value.trim() !== '') {
      ...
      if (field.fieldType === 'email') {
        ...
        if (field.validation?.blockedEmailDomains && field.validation.blockedEmailDomains.length > 0) {
          const domain = trimmedValue.split('@')[1]?.toLowerCase();
          if (domain && field.validation.blockedEmailDomains.some(blocked => 
      ...
      if (field.fieldType === 'mobile_phone') {
        ...
        if (field.validation?.minAllowedDigits && digitsOnly.length < field.validation.minAllowedDigits) {
          errors.push(`Phone number must contain at least ${field.validation.minAllowedDigits} digits.`);
        }
        
        if (field.validation?.maxAllowedDigits && digitsOnly.length > field.validation.maxAllowedDigits) {
          errors.push(`Phone number must contain no more than ${field.validation.maxAllowedDigits} digits.`);
        }
          ...
          switch (field.validation.name) {
            case 'minLength':
              if (field.validation.data && trimmedValue.length < parseInt(field.validation.data)) {
                errors.push(field.validation.message ?? `Must be at least ${field.validation.data} characters.`);
              }
              break;
            case 'maxLength':
              if (field.validation.data && trimmedValue.length > parseInt(field.validation.data)) {
                errors.push(field.validation.message ?? `Must be no more than ${field.validation.data} characters.`);
              }
              break;
            case 'regex':
