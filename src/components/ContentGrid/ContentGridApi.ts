...
export async function getContentGridById(id: string, preview = false): Promise<ContentGrid | null> {
  try {
    ...
    if (
      contentGrid.itemsCollection?.items?.length &&
      contentGrid.itemsCollection.items.length > 0
    ) {
      console.warn(
        'ContentGrid: Starting PARALLEL enrichment for',
        contentGrid.itemsCollection.items.length,
        'items'
      );
      console.warn(
      ...
      console.warn(
        'ContentGrid: Checking for Slider items. All item types:',
        contentGrid.itemsCollection.items.map((item) => item.__typename)
      );
      const sliderItems = contentGrid.itemsCollection.items.filter(
        (item) => item.__typename === 'Slider'
      );
      if (sliderItems.length > 0) {
        console.warn(
          'ContentGrid: Found Slider items:',
        ...
        console.warn('ContentGrid: Available item types are:', [
          ...new Set(contentGrid.itemsCollection.items.map((item) => item.__typename))
        ]);
      }
      ...
      const collectionIdsPromise = hasEmptyObjects
        ? getCollectionIdsFromContentGrid(id)
            .then((ids) => {
      ...
      const enrichmentPromises = contentGrid.itemsCollection.items.map(
        async (item: any, index: number) => {
          console.log(`ContentGrid: Processing item ${index}:`, {
            id: item.sys?.id,
            typename: item.__typename,
          ...
          if (item && typeof item === 'object' && Object.keys(item).length === 0) {
            try {
              ...
              if (collectionId) {
                try {
                  const { getCollectionById } = await import(
                    '@/components/Collection/CollectionApi'
                  );
                  console.warn(
                    'ContentGrid: Enriching Collection (empty object) PARALLEL',
                    collectionId
                  );
                  ...
                  return (
                    enrichedCollection || { sys: { id: collectionId }, __typename: 'Collection' }
                  );
                } catch (error) {
                  console.warn(
                    `Failed to enrich Collection ${collectionId} in ContentGrid:`,
                    error
                  );
          ...
          if (item.__typename === 'Slider' && item.sys?.id) {
            ...
            try {
              ...
              console.warn(
                `Failed to enrich ContentSliderItem ${item.sys.id} in ContentGrid:`,
                error
              );
              return item;
            }
            ...
            try {
              const { getTestimonialsById } = await import(
                '@/components/Testimonials/TestimonialsApi'
              );
            ...
            try {
              ...
              console.warn(
                `Failed to enrich ContentGridItem ${item.sys.id} in ContentGrid:`,
                error
              );
              return item;
            }
            ...
            try {
              ...
              console.log(`ContentGrid: ContactCard ${item.sys.id} enriched successfully:`, {
                hasTitle: !!enrichedItem?.title
              });
              return enrichedItem || item;
            } catch (error) {
            ...
            try {
              const { getLocationById } = await import(
                '@/components/OfficeLocation/OfficeLocationApi'
              );
      ...
      console.log(
        `ContentGrid: Enrichment completed. Original items:`,
        contentGrid.itemsCollection?.items?.length
      );
      console.log(`ContentGrid: Enriched items:`, enrichedItems.length);
      console.log(
        `ContentGrid: Enriched ContactCards:`,
        enrichedItems
          .filter((item) => item.__typename === 'ContactCard')
          .map((item) => ({ id: item.sys?.id, hasTitle: !!item.title }))
      );
            ...
            if (item.__typename === 'Slider' && item.sys?.id) {
              ...
              try {
                ...
                if (enrichedSlider) {
                  ...
                  console.warn(
                    'ContentGrid Collection: getSliderById returned null, returning original item'
                  );
                  return item;
                }
              } catch (error) {
                console.warn(
                  `ContentGrid Collection: Failed to enrich Slider ${item.sys.id}:`,
                  error
                );
                return item; // Return original item on error
              }
              ...
              try {
                ...
                console.warn(
                  `Failed to enrich Post ${item.sys.id} in ContentGrid collection:`,
                  error
                );
                return item; // Return original item on error
              }
              ...
              try {
                ...
                console.warn(
                  `Failed to enrich CtaGrid ${item.sys.id} in ContentGrid collection:`,
                  error
                );
                return item;
              }
              ...
              try {
                ...
                console.warn(
                  `Failed to enrich Profile ${item.sys.id} in ContentGrid collection:`,
                  error
                );
                return item;
              }
              ...
              try {
                ...
                console.warn(
                  `Failed to enrich Accordion ${item.sys.id} in ContentGrid collection:`,
                  error
                );
                return item;
              }
              ...
              try {
                ...
                console.warn(
                  `Failed to enrich ContentSliderItem ${item.sys.id} in ContentGrid collection:`,
                  error
                );
                return item;
              }
              ...
              try {
                ...
                console.warn(
                  `Failed to enrich Testimonials ${item.sys.id} in ContentGrid collection:`,
                  error
                );
                return item;
              }
              ...
              try {
                ...
                console.warn(
                  `Failed to enrich Product ${item.sys.id} in ContentGrid collection:`,
                  error
                );
                return item;
              }
              ...
              try {
                ...
                console.warn(
                  `Failed to enrich Service ${item.sys.id} in ContentGrid collection:`,
                  error
                );
                return item;
              }
              ...
              try {
                ...
                console.warn(
                  `Failed to enrich Solution ${item.sys.id} in ContentGrid collection:`,
                  error
                );
                return item;
              }
              ...
              try {
                ...
                console.warn(
                  `Failed to enrich Event ${item.sys.id} in ContentGrid collection:`,
                  error
                );
                return item;
              }
              ...
              try {
                ...
                const enrichedItem = await getContactCardById(item.sys.id, preview);
                console.log(
                  `ContentGrid Collection: ContactCard ${item.sys.id} enriched successfully:`,
                  { hasTitle: !!enrichedItem?.title }
                );
                return enrichedItem || item;
              } catch (error) {
                console.warn(
                  `Failed to enrich ContactCard ${item.sys.id} in ContentGrid collection:`,
                  error
                );
                return item;
              }
              ...
              try {
                ...
                console.warn(
                  `Failed to enrich Image ${item.sys.id} in ContentGrid collection:`,
                  error
                );
                return item;
              }
              ...
              try {
                ...
                console.warn(
                  `Failed to enrich OfficeLocation ${item.sys.id} in ContentGrid collection:`,
                  error
                );
                return item;
              }
              ...
              try {
                ...
                console.warn(
                  `Failed to enrich PageList ${item.sys.id} in ContentGrid collection:`,
                  error
                );
                return item;
              }
              ...
              try {
                ...
                console.warn(
                  `Failed to enrich ContentGridItem ${item.sys.id} in ContentGrid collection:`,
                  error
                );
                return item;
              }
...
export async function getContentGridItemById(
  ...
  try {
    ...
    if (contentGrid) {
      console.warn(
        'ContentGrid: getContentGridItemById found ContentGrid, returning enriched version'
      );
      // Transform ContentGrid to ContentGridItem format
      return {
        ...contentGrid,
        heading:
          typeof contentGrid.heading === 'object'
