...
export function SearchCard(props: SearchCardProps) {
  const [itemData, setItemData] = useState<Page | PageList | Post | Product | Solution | Service | null>(null);
  ...
  const item = useContentfulLiveUpdates(itemData ?? (props as Page | PageList | Post | Product | Solution | Service));
  ...
  const imageSource = getImageSourceForContentType(item, props.contentType) as { link?: string; altText?: string } | null;
      ...
      <div className="flex items-start gap-4 p-4 border-b border-gray-200 hover:bg-gray-50 transition-colors">
        ...
        <div className="flex-1 min-w-0">
          ...
          {(() => {
            ...
            const isValidDescription = typeof description === 'string' || typeof description === 'number';
